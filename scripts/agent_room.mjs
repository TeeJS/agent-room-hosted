...
fetch('/api/rooms?status=open').then(function(r){if(!r.ok)throw new Error('unavailable');return r.json();}).then(function(d){var list=(d&&d.rooms)||[];sec.textContent='';sec.appendChild(el('div','rooms-label','Active rooms · '+list.length));if(!list.length){sec.appendChild(el('p','rooms-empty','No active rooms right now.'));}list.forEach(function(room){var a=el('a','room');a.href='/rooms/'+encodeURIComponent(String(room.code||''));a.appendChild(el('div','room-title',String(room.title||'Untitled')));var when='';if(room.updated_at){var t=new Date(room.updated_at);if(!isNaN(t))when=t.toLocaleString();}a.appendChild(el('div','room-meta',String(room.code||'')+' · '+(Number(room.participant_count)||0)+' in room · '+(Number(room.active_agents)||0)+' agents'+(when?' · '+when:'')));sec.appendChild(a);});sec.hidden=false;}).catch(function(){sec.hidden=true;});})();</script></body></html>`;
}
