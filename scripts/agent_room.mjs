...
async function jsonOf(response){const ct=response.headers.get('content-type')||'';if(response.type==='opaqueredirect'||response.status===401||!ct.includes('json')){expired();const e=new Error('Session expired — sign in again');e.expired=true;throw e}return response.json()}
...
let failures=0;
async function poll(){let closed=false;let delay=350;try{const response=await F(\`/api/rooms/\${code}/messages?after=\${cursor}&wait=25\`);const data=await jsonOf(response);if(!response.ok)throw new Error(data.error||'HTTP '+response.status);failures=0;hideBanner();markUpdated();el('status').textContent=data.status;const active=data.messages.filter(m=>m.kind==='agent'||m.kind==='human').at(-1)?.sender;renderMode(data.addressed_only);renderPeople(data.participants||[],active);renderRemoved(data.removed||ROOM.removed);data.messages.forEach(m=>addMessage(m));closed=data.status==='closed';if(closed){el('message').disabled=true;el('send').disabled=true;el('mode').disabled=true;el('closeRoom').disabled=true;renderPeople(data.participants||[],active);renderRemoved(data.removed||ROOM.removed)}}catch(error){if(error.expired)return;failures+=1;delay=Math.min(15000,1000*failures);showBanner('Connection lost ('+error.message+'). Retrying… New messages are not being received.','Reload',()=>location.reload())}if(!closed)setTimeout(poll,delay)}
function toast(message){el('toast').textContent=message;el('toast').classList.add('show');setTimeout(()=>el('toast').classList.remove('show'),1800)}async function sendViewerMessage(){const content=el('message').value.trim();if(!content)return;el('send').disabled=true;try{const response=await F(\`/api/rooms/\${code}/viewer/messages\`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({content})});const data=await jsonOf(response);if(!response.ok)throw new Error(data.error||'Could not send message');el('message').value='';renderPeople(data.participants||ROOM.participants,data.message.sender)}catch(error){toast(error.message)}finally{if(el('status').textContent!=='closed')el('send').disabled=false}}
