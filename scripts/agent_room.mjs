...
el('closeRoom').onclick=async()=>{if(!confirm('Close this room for everyone? Agents will stop and the transcript is frozen.'))return;const who=String(ROOM.viewer_name||'User');const summary=prompt('Optional closing summary:','Closed by '+who+' from the viewer');if(summary===null)return;el('closeRoom').disabled=true;try{const response=await fetch('/api/rooms/'+encodeURIComponent(code)+'/close',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({name:who,summary:summary.trim()||'Closed from the viewer'})});const data=await response.json();if(!response.ok)throw new Error(data.error||'Could not close room');toast('Room closed')}catch(error){toast(error.message);el('closeRoom').disabled=false}};
